...
def decide_strategy(schema: dict, schema_queries: SchemaQueries):
...
def create_caching_coordinator(
...
def main(page_url: str, schema_files:dict):
    ...
    driver = load_driver(page_url)
    caching_coordinator = create_caching_coordinator(
        schema_queries,
        config_queries,
        template_registry
    )
    #here we call the driver: setting up 
    root_element = get_root_web_element(main_schema, driver, schema_queries)
    caching_coordinator.initialize_with_root(root_element)
    strategy = decide_strategy(schema_queries, main_schema)
